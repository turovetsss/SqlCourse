...
import { fetchTypes, fetchOneFunc, updateFunc } from "../../http/itemAPI";
import './CreateFunc.css'

const EditFunc = observer(({ show, onHide, funcId }) => {
  ...
  useEffect(() => {
    ...
    fetchOneFunc(funcId).then(data => {
      setType(data.funcType);
      setName(data.name);
  ...
  return (
    ...
    >
            ...
            <Dropdown.Menu>
              {course.types.map(type =>
                <Dropdown.Item
                  ...
                  key={type.id}
                  selected={type.id === course.selectedType.id} 
                >
                  {type.name}
                </Dropdown.Item>
              )}
      ...
      <Modal.Footer>
        ...
        <Button variant="outline-success" onClick={updateFuncData}>Обновить</Button>
      </Modal.Footer>
    </Modal>
  );
})
